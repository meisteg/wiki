 ...
 * $Id: WikiDiff.php,v 1.5 2006/01/02 21:30:42 blindman1344 Exp $
 */

require_once 'Text/Diff.php';
require_once 'Text/Diff/Renderer.php';

class PHPWS_WikiDiff extends Text_Diff_Renderer {

    /**
     * Holds the output during the render
     *
     * @var string
     */
    var $_output = NULL;
     ...
     * @author Greg Meiste <blindman1344@NOSPAM.users.sourceforge.net>
     */
    function PHPWS_WikiDiff($lines=3)
    {
     ...
     * @author Greg Meiste <blindman1344@NOSPAM.users.sourceforge.net>
     */
    function diff($oid, $nid)
    {
        require_once PHPWS_SOURCE_DIR . 'mod/wiki/class/OldWikiPage.php';
        $olderpage =& new PHPWS_OldWikiPage($oid);
        $newerpage =& new PHPWS_OldWikiPage($nid);
        ...
        $tags = array();
        $tags['TITLE'] = $_SESSION['translate']->it('Difference between revisions');
        $tags['BACK_PAGE'] = '<a href="./index.php?module=wiki&amp;page=' . $_REQUEST['page'] . '">' . $_SESSION['translate']->it('Back to Page') . '</a>';
        $tags['BACK_HISTORY'] = '<a href="./index.php?module=wiki&amp;page_op=history&amp;page=' . $_REQUEST['page'] . '">' . $_SESSION['translate']->it('Back to History') . '</a>';
        $tags['DIFF'] = $this->_output;
        $tags['OLDER_VERSION'] = $olderpage->getListVersion();
        $tags['NEWER_VERSION'] = $newerpage->getListVersion();
        $tags['OLDER_UPDATED'] = $olderpage->getListUpdated();
        $tags['NEWER_UPDATED'] = $newerpage->getListUpdated();
        $tags['VERSION_LABEL'] = $_SESSION['translate']->it('Version');

        return PHPWS_Template::processTemplate($tags, 'wiki', 'diff/diff.tpl');
    }
    ...
    function _blockHeader($xbeg, $xlen, $ybeg, $ylen)
    {
        ...
        $this->_output .= PHPWS_Template::processTemplate($tags, 'wiki', 'diff/blockheader.tpl');
    }
    ...
    function _lines($lines, $template)
    {
        foreach ($lines as $line) {
            if($line == NULL) {
                $this->_output .= PHPWS_Template::processTemplate(array('LINE'=>'&nbsp;'), 'wiki', 'diff/' . $template . '.tpl');
            }
            else {
                $this->_output .= PHPWS_Template::processTemplate(array('LINE'=>$line), 'wiki', 'diff/' . $template . '.tpl');
            }
        }
    }
